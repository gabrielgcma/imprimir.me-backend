...
class PedidoCrud:
    ...
    def cadastrar_pedido(self, pedido: PedidoCreate):
        pedido_decode = pedido.model_dump_json()
        try:
            publisher = Publisher()
            logging.info("Enviando documento do pedido para o broker...")
            try:
                publisher.publish(pedido_decode.encode())
            except Exception as e:
                logging.error("Erro ao enviar documento do pedido para o broker: " + e)
        except Exception as e:
            logging.error(
                "Erro ao registrar novo Pedido no banco - services/pedidoservice.py - cadastrar_pedido() - "
                + str(pedido_decode)
                + str(e)
            )
